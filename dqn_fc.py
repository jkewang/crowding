...
with tf.variable_scope('q'):
    b1w = tf.Variable(tf.random_normal([N_SLIDING,256],name="b1w"))
    b1b = tf.Variable(tf.zeros([1,256]),name="b1b")
    b1lo = tf.matmul(tf_s_sliding,b1w) + b1b
    b1o = tf.nn.relu(b1lo)
    b2w = tf.Variable(tf.random_normal([256,256]),name="b2w")
    b2b = tf.Variable(tf.zeros([1,256]),name="b2b")
...
#saver.restore(sess,"./model/my-model.ckpt-3500")
